...
{   // initialize some vars
    ...
    {   // regexes to use
        $regex_windows  = '/(windows)[\s]?([0-9a-z]*)[\w\s]?([a-z0-9.]*)/i';
        $regex_mac      = '/(68[k0]{1,3})|(ppc mac os x)|([p\S]{1,5}pc)|(darwin)|(intel mac os x)/i';
        ...
        $regex_ios      = '/CPU [iPhone\s]*OS ([\d\_]+) like Mac OS X/i';
        $regex_android  = '/Android ([\d\.\-a-z]+)/i';
        $regex_webos    = '/webos\/([\d\.]+)/i';
        $regex_symbian  = '/symbianos\/([\d\.]+)/i';
        $regex_bberry   = '/blackberry/i';
        ...
        {   /** Windows has some of the most ridiculous HTTP_USER_AGENT strings */
            ...
            // Establish NT 5.1 as Windows XP
				...
				elseif(stristr($v,'NT') && $v2 == 5.2) $v = 'xp 64-bit';
				elseif(stristr($v,'NT') && $v2 == 6.1) $v = '7';
				elseif(stristr($v,'NT') && $v2 == 6.0) $v = 'vista';
        ...
        elseif(preg_match($regex_bberry,$this->_browser_info['ua'],$match))
        {   $this->_set_browser('platform','blackberry');
            $this->_set_browser('os','');
        }
        // look for mac
        ...
        {   $this->_set_browser('platform','ios');
            $this->_set_browser('os',str_replace('_', '.', $match[1]));
        }
        elseif(preg_match($regex_webos,$this->_browser_info['ua'],$match))
        {   $this->_set_browser('platform','webos');
            $this->_set_browser('os',$match[1]);
        }
        elseif(preg_match($regex_symbian,$this->_browser_info['ua'],$match))
        {   $this->_set_browser('platform','symbian');
            $this->_set_browser('os',$match[1]);
        }
    }
    ...
    {   $this->_build_regex();
        ...
        {   // get the position of the last browser found
            ...
            if (preg_match('/chrome/i',$this->_browser_info['ua'])) $count--;
            elseif($this->_allow_masquerading && $count > 0) $count--;
            ...
            if ($this->_browser_info['long_name'] == 'safari' && preg_match('/Version\/(\d+)([\.\d]+)/i', $this->_browser_info['ua'], $results)) {
                $this->_set_browser('maj_ver',$results[1]);
                $this->_set_browser('min_ver',$results[2]);
            }
