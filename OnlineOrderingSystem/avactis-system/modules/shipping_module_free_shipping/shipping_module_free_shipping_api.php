...
/**
 * Shipping Module "Free Shipping".
 *
 * @package ShippingModuleFreeShipping
 * @author Ravil Garafutdinov
 */
class Shipping_Module_Free_Shipping extends pm_sm_api
{
//------------------------------------------------
//               PUBLIC DECLARATION
//------------------------------------------------
     ...
     */
    function Shipping_Module_Free_Shipping()
    {
        $this->Settings = null;
    }
    ...
    function getUID()
    {
        return "F3B38526-3910-40D4-869B-0A7133A9CABE";
    }
    ...
    {
        global $application;
        $obj = &$application->getInstance('MessageResources',"shipping-module-free-shipping-messages", "AdminZone");
        ...
        return array("GlobalUniqueShippingModuleID" => $this->getUID(),
                     ...
                     "PreferencesAZViewClassName" => NULL, //"free_shipping_input_az",
    ...
    function getShippingMethodInfo($method_id, $and_calc=false)
    {
        if($method_id == $this->single_available_method_id)
        {
            ...
            return array(
                        ...
                        'method_code' => '',
                        'destination' => 'L',
                        'available' => 'Y',//$this->IsActive() ? 'Y' : 'N',
                        'cost' => '0.0'//( $and_calc ? $this->getShippingCost() : PRICE_N_A )
                        ,'days' => null
                        ,'shipping_cost' => '0.0'
            );
        }
    ...
    function install()
    {
        ...
        $tables = Shipping_Module_Free_Shipping::getTables();
        $query = new DB_Table_Create($tables);

        $table = 'sm_free_shipping_settings';            #
    ...
    function getTables()
    {
        ...
        $settings = 'sm_free_shipping_settings';
        $tables[$settings] = array();
        $tables[$settings]['columns'] = array
            (
                'id'                => $settings.'.sm_free_shipping_setting_id'
               ,'key'               => $settings.'.sm_free_shipping_setting_key'
               ,'value'             => $settings.'.sm_free_shipping_setting_value'
            );
    ...
    {
        global $application;
        $query = new DB_Delete('sm_free_shipping_settings');
        $application->db->getDB_Result($query);
    }
    ...
    {
        $result = execQuery('SELECT_PM_SM_SETTINGS',array('ApiClassName' => __CLASS__, 'settings_table_name' => 'sm_free_shipping_settings'));
        $this->Settings = array();
    ...
    function setSettings($Settings)
    {
        ...
        $columns = $tables['sm_free_shipping_settings']['columns'];

        foreach($Settings as $key => $value)
        {
            $query = new DB_Insert('sm_free_shipping_settings');
