...
class Mail_RFC822
{
    ...
    */
    ...
    function _splitAddresses($address)
    {
        ...
        $address = trim(_ml_substr($address, _ml_strlen($string) + 1));

        // If the next char is a comma and this was a group, then
        // there are more addresses, otherwise, if there are any more
        // chars, then there is another address.
        if ($is_group && _ml_substr($address, 0, 1) == ','){
            $address = trim(_ml_substr($address, 1));
            return $address;

        } elseif (_ml_strlen($address) > 0) {
            return $address;
    ...
    function _hasUnclosedBracketsSub($string, &$num, $char)
    {
        ...
        for ($i = 0; $i < count($parts); $i++){
            if (_ml_substr($parts[$i], -1) == '\\' || $this->_hasUnclosedQuotes($parts[$i]))
                $num--;
            if (isset($parts[$i + 1]))
    ...
    function _validateAddress($address)
    {
        ...
        if ($address['group']) {
            ...
            $address['address'] = ltrim(_ml_substr($address['address'], _ml_strlen($groupname . ':')));
        }

        // If a group then split on comma and put into an array.
        // Otherwise, Just put the whole address in an array.
        if ($is_group) {
            while (_ml_strlen($address['address']) > 0) {
                $parts       = explode(',', $address['address']);
                $addresses[] = $this->_splitCheck($parts, ',');
                $address['address'] = trim(_ml_substr($address['address'], _ml_strlen(end($addresses) . ',')));
            }
        } else {
    ...
    function _validatePhrase($phrase)
    {
        ...
        for ($i = 0; $i < count($phrase_parts); $i++) {
            // If quoted string:
            if (_ml_substr($phrase_parts[$i], 0, 1) == '"') {
    ...
    function _validateQuotedString($qstring)
    {
        ...
        $qstring = _ml_substr($qstring, 1, -1);
    ...
    function validateMailbox(&$mailbox)
    {
        ...
        $_mailbox = $mailbox;
        while (_ml_strlen(trim($_mailbox)) > 0) {
            $parts = explode('(', $_mailbox);
            $before_comment = $this->_splitCheck($parts, '(');
            if ($before_comment != $_mailbox) {
                // First char should be a (
                $comment    = _ml_substr(str_replace($before_comment, '', $_mailbox), 1);
                $parts      = explode(')', $comment);
                $comment    = $this->_splitCheck($parts, ')');
                $comments[] = $comment;

                // +1 is for the trailing )
                $_mailbox   = _ml_substr($_mailbox, _ml_strpos($_mailbox, $comment)+_ml_strlen($comment)+1);
            } else {
                break;
            }
        }
        ...
        if (_ml_substr($mailbox, -1) == '>' && _ml_substr($mailbox, 0, 1) != '<') {
            $parts  = explode('<', $mailbox);
            $name   = $this->_splitCheck($parts, '<');

            $phrase     = trim($name);
            $route_addr = trim(_ml_substr($mailbox, _ml_strlen($name.'<'), -1));
        ...
        // Only got addr-spec
            ...
            if (_ml_substr($mailbox,0,1) == '<' && _ml_substr($mailbox,-1) == '>')
                $addr_spec = _ml_substr($mailbox,1,-1);
            else
                $addr_spec = $mailbox;
    ...
    {
        // Check for colon.
        if (_ml_strpos($route_addr, ':') !== false) {
        ...
        if ($route === $route_addr){
            ...
            $addr_spec = _ml_substr($route_addr, _ml_strlen($route . ':'));
    ...
    function _validateAddrSpec($addr_spec)
    {
        ...
        if (_ml_strpos($addr_spec, '@') !== false) {
            $parts      = explode('@', $addr_spec);
            $local_part = $this->_splitCheck($parts, '@');
            $domain     = _ml_substr($addr_spec, _ml_strlen($local_part . '@'));
