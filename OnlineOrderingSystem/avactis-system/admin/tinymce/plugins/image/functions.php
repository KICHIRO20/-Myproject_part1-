...
function DoUpload($field = 'userfile'){
	...
	if(!CanAcessUploadForm()){
		$output["reason"] = _lang('no_permission_to_upload');
		$output["success"] = false;
		return $output;
	}

	if(!isset($_FILES[$field])){
		$output["reason"] = _lang('file_not_selected');
		$output["success"] = false;
		return $output;
	}
	...
	if(!is_uploaded_file($_FILES[$field]['tmp_name'])){
		...
		switch($error){
			case 1:	// UPLOAD_ERR_INI_SIZE
				$output["reason"] = _lang('file_exceeds_limit_size');
				break;
			case 2: // UPLOAD_ERR_FORM_SIZE
				$output["reason"] = _lang('file_exceeds_limit_size');
				break;
			case 3: // UPLOAD_ERR_PARTIAL
				$output["reason"] = _lang('file_uploaded_partially');
				break;
			case 4: // UPLOAD_ERR_NO_FILE
				$output["reason"] = _lang('file_not_selected');
				break;
			case 6: // UPLOAD_ERR_NO_TMP_DIR
				$output["reason"] = _lang('no_temp_directory');
				break;
			case 7: // UPLOAD_ERR_CANT_WRITE
				$output["reason"] = _lang('unable_to_write_the_file');
				break;
			case 8: // UPLOAD_ERR_EXTENSION
				$output["reason"] = _lang('invalid_extension');
				break;
			default :
				$output["reason"] = _lang('file_not_selected');
				break;
	...
	if(!@copy($_FILES[$field]['tmp_name'], $current_folder.$file_name)){
		if(!@move_uploaded_file($_FILES[$field]['tmp_name'], $current_folder.$file_name)){
			$output["reason"] = _lang('could_not_move_file');
			$output["success"] = false;
			return $output;
		}
	}
