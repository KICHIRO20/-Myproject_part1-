...
class JsHttpRequest
{
    ...
    function _correctSuperglobals()
    {
        ...
        $rawPost = _ml_strcasecmp($_SERVER['REQUEST_METHOD'], 'POST') == 0? (isset($GLOBALS['HTTP_RAW_POST_DATA'])? $GLOBALS['HTTP_RAW_POST_DATA'] : @file_get_contents("php://input")) : null;
        $source = array(
    ...
    function _decUcs2Decode($code, $toEnc)
    {
        ...
        if (isset($this->_encTables[$toEnc])) {
            // : possible speedup by using array_flip($this->_encTables) and later hash access in the constructor.
