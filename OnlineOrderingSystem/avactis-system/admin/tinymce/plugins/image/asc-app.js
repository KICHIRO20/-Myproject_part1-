$(document).ready(function(){
	...
	$("#cat-select-image").click(function(){
		//$(window.parent+"#ViewState_SmallImage").attr('value',$);
		closeAndFocusParent();
	});
	...
	$("#search").focus(function () {
		...
		$.getJSON('search.php?catalog=yes',{}, function(returned){ 
	...
	$(document).on('click', 'a#clear-search', function () {
		...
		$.getJSON('lib.php' + '?catalog=yes&dummy=' + new Date().getTime(),{}, function(returned){ 
	...
	$("#search").keyup(function(event) {
			...
			if(results.length > 0){
				for(i=0;i<results.length;i++){
					//$('#gallery-images').append('<a href="" class="img-thumbs" rel="' + results[i][0] + '"><img src="asc-thumb.php?src=' + results[i][0] + '&w=130&h=90" class="img-polaroid" width="130" height="90"></a>');
					$('#gallery-images').append('<div class="item"><a href="" class="img-thumbs" rel="' + results[i][0] + '" title="' + results[i][1] + '"><img src="asc-thumb.php?catalog=yes&src=' + results[i][0] + '&w=130&h=90" class="img-polaroid" width="130" height="90"></a><div><a href="" class="pull-left transparent change-file" title="' + trans_change_name + '" rel="' + results[i][1] + '"><i class="icon-pencil"></i></a><a href="" class="pull-right transparent delete-file" data-path="'+ results[i][3] +'" rel="'+ results[i][2] +'" title="' + trans_delete + '"><i class="icon-trash"></i></a><div class="clearfix"></div></div></div>');
				}
			}else{
	...
	$("#preview").bind("load", function () {
		if(newImage){
			...
			parent.document.getElementById("cat-width").value= originalWidth;
			parent.document.getElementById("cat-height").value= originalHeight;
		}else{
			newImage = true;
			if ($("#preview").get(0).naturalWidth) {
				originalWidth = $("#preview").get(0).naturalWidth;
	...
	$(document).on('click', 'a.img-thumbs', function () {
		...
		$("#source").val($(this).attr("rel"));
        	$("#preview").attr("src", $(this).attr("rel") + '?catalog=yes&dummy=' + new Date().getTime());
        	$('#myTab a[href="#tab1"]').tab('show');
        	parent.document.getElementById("ViewState_"+forimage).value= $(this).attr("rel");
			$(parent.document).find('#cat-' + forimage).attr('src', $(this).attr("rel"));
        	$.post("update_recent.php" + "?catalog=yes&dummy=" + new Date().getTime(), { src: $(this).attr("rel") } );
			closeAndFocusParent();
		return false;
	});
	...
	$(document).on('click', 'a.pdf-thumbs', function () {
        ...
        $('#myTab a[href="#tab1"]').tab('show');
        parent.document.getElementById("cat-src").value= $(this).attr("rel");
        $.post("update_recent.php" + "?catalog=yes&dummy=" + new Date().getTime(), { src: $(this).attr("rel") } );
		return false;
	});
	
	$("#class_pdf").bind("change", function () {
		parent.document.getElementById("cat-class").value= this.value;
	});
	
	$("#title_pdf").bind("change", function () {
		parent.document.getElementById("cat-title").value= this.value;
	});
	
	$("#target_pdf").bind("change", function () {
		parent.document.getElementById("cat-target").value= this.value;
	});
	
	$("#source").bind("change", function () {
		$.post("update_recent.php" + "?catalog=yes&dummy=" + new Date().getTime(), { src: this.value } );
		$("#preview").attr("src", this.value + '?catalog=yes&dummy=' + new Date().getTime());
		parent.document.getElementById("cat-src").value= this.value;
	});
	
	$("#alt").bind("change", function () {
		parent.document.getElementById("cat-alt").value= this.value;
	});
	...
	$("#width").keyup(function(event) {
    		parent.document.getElementById("cat-width").value= this.value;
		if($('#constrain').is(':checked') && this.value != originalWidth){
			parent.document.getElementById("cat-height").value= Math.round((this.value / originalWidth) * originalHeight);
    	...
    	$("#height").keyup(function(event) {
    		parent.document.getElementById("cat-height").value= this.value;
		if($('#constrain').is(':checked') && this.value != originalHeight){
			parent.document.getElementById("cat-width").value= Math.round((this.value / originalHeight) * originalWidth);
	...
	$("#do_link").bind("change", function () {
		if($(this).is(':checked')){
			...
			parent.document.getElementById("cat-linkURL").value= "";
			
			$("#target").attr('disabled','disabled');
			parent.document.getElementById("cat-target").value= "";
		}
	});
	
	$("#link_url").bind("change", function () {
		parent.document.getElementById("cat-linkURL").value= this.value;
	});
	...
	$("#float").bind("change", function () {
		parent.document.getElementById("cat-align").value= this.value;
	});
	...
	$("#get-recent").bind("click", function () {
		...
		$.getJSON('recent.php?catalog=yes',{}, function(returned){ 
			if(returned.success == 1){
	...
	$("#refresh").bind("click", function () {
		...
		$('#gallery-images').empty().append('<div id="ajax-loader-div"><img src="bootstrap/img/ajax-loader.gif" alt="Loading..." class="ajax-loader"></div>');
		$.getJSON('lib.php' + '?catalog=yes&dummy=' + new Date().getTime(),{path: $(this).attr("rel")}, function(returned){ 
			if(returned.success == 1){
				$('#gallery-images').empty().append(returned.html);
			}else{
	...
	$("#toggle-layout").bind("click", function () {
		...
		$('#gallery-images').empty().append('<div id="ajax-loader-div"><img src="bootstrap/img/ajax-loader.gif" alt="Loading..." class="ajax-loader"></div>');
		$.getJSON('lib.php' + '?catalog=yes&dummy=' + new Date().getTime(),{path: $(this).attr("rel"), toggle: 1}, function(returned){ 
			if(returned.success == 1){
				$('#gallery-images').empty().append(returned.html);
			}else{
				$('#gallery-images').empty().append('<center>' + trans_no_images_in_folder + '</center>');
	...
	$("#get-lib").bind("click", function () {
		if(loaded == false){
			...
			$('#gallery-images').empty().append('<div id="ajax-loader-div"><img src="bootstrap/img/ajax-loader.gif" alt="Loading..." class="ajax-loader"></div>');
			$.getJSON('lib.php' + '?catalog=yes&dummy=' + new Date().getTime(),{path: $("#refresh").attr("rel")}, function(returned){ 
				if(returned.success == 1){
					$('#gallery-images').empty().append(returned.html);
				}else{
					$('#gallery-images').empty().append('<center>' + trans_no_images_in_folder + '</center>');
	...
	$(document).on('click', '#newfolder_btn', function () {
		...
		$.getJSON('new_folder.php' + '?catalog=yes&dummy=' + new Date().getTime(),{path: $("#refresh").attr("rel"), folder: $('#newfolder_name').val()}, function(returned){ 
			if(returned.success == 1){
				$('#newfolder_name').val("");
				$('#gallery-images').empty().append(returned.html);
	...
	$(document).on('click', 'a.delete-file', function () {
		...
		$.getJSON('delete_file.php' + '?catalog=yes&dummy=' + new Date().getTime(),{path: $(this).data("path"),file: $(this).attr("rel")}, function(returned){ 
			if(returned.success == 1){
				$('#gallery-images').empty().append(returned.html);
			}else{
				the_parent.empty();
	...
	$(document).on('click', 'a.delete-folder', function () {
		...
		$.getJSON('delete_folder.php' + '?catalog=yes&dummy=' + new Date().getTime(),{path: $("#refresh").attr("rel"),folder: $(this).attr("rel")}, function(returned){ 
			if(returned.success == 1){
				$('#gallery-images').empty().append(returned.html);
			}else{
				the_parent.empty();
	...
	$(document).on('click', 'a.change-folder', function () {
		...
		$.getJSON('rename_folder.php' + '?catalog=yes&dummy=' + new Date().getTime(),{path: $("#refresh").attr("rel"),new_name: r,current_name: current_value}, function(returned){ 
			if(returned.success == 1){
				$('#gallery-images').empty().append(returned.html);
			}else{
				the_parent.empty();
	...
	$(document).on('click', 'a.change-file', function () {
		...
		$.getJSON('rename_file.php' + '?catalog=yes&dummy=' + new Date().getTime(),{path: $("#refresh").attr("rel"),new_name: (r + "." + extension),current_name: current_value}, function(returned){ 
			if(returned.success == 1){
				$('#gallery-images').empty().append(returned.html);
			}else{
				the_parent.empty();
	...
	$(document).on('click', '#refresh-dirs', function () {
		...
		$.getJSON('refresh_dir_list.php' + '?catalog=yes&dummy=' + new Date().getTime(),{}, function(returned){ 
			if(returned.success == 1){
	...
	$(document).on('change', '#select-dir', function () {
		...
		$.getJSON('set_upload_directory.php' + '?catalog=yes&dummy=' + new Date().getTime(),{path:$(this).val() }, function(returned){ 
			if(returned.success == 1){
				$('#select-dir-msg').empty().append('<span style="color: green;">Done...&nbsp;&nbsp;&nbsp;</span>');
				setTimeout(function(){ $('#select-dir-msg').empty() }, 5000);
			}
	...
	$(document).on('click', 'a.page-link', function () {
		...
		$.getJSON('lib.php' + '?catalog=yes&dummy=' + new Date().getTime(),{path: $(this).data("path"),page: $(this).data("page")}, function(returned){ 
			if(returned.success == 1){
				$('#gallery-images').empty().append(returned.html);
			}else{
				$('#gallery-images').empty().append('<center>' + trans_no_images_in_folder + '</center>');
