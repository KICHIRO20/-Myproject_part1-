...
class Quick_Books
{
    ...
    function uninstall()
    {
        $query = new DB_Table_Delete(Quick_Books::getTables());
    }
    ...
    function getSettings()
    {
        ...
        $query = new DB_Select();
        $query->addSelectTable('qb_settings');
    ...
    function updateSettings($settings)
    {
        global $application;
        $tables=$this->getTables();
        $stable=$tables['qb_settings']['columns'];

        foreach($settings as $skey => $sval)
        {
            $query = new DB_Update('qb_settings');
