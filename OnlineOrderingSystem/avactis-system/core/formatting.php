 ...
 *
 * @since 0.71
 * @uses $asc_cockneyreplace Array of formatted entities for certain common phrases
 *
...
function wptexturize($text, $reset = false) {
	global $asc_cockneyreplace, $shortcode_tags;
	...
	if ( $reset || ! isset( $static_characters ) ) {
		...
		if ( isset($asc_cockneyreplace) ) {
			$cockney = array_keys($asc_cockneyreplace);
			$cockneyreplace = array_values($asc_cockneyreplace);
 ...
 */
function _asc_specialchars( $string, $quote_style = ENT_NOQUOTES, $charset = false, $double_encode = false ) {
	...
	// Store the site charset as a static to avoid multiple calls to asc_load_alloptions()
	if ( ! $charset ) {
		static $_charset;
		if ( ! isset( $_charset ) ) {
			$alloptions = asc_load_alloptions();
	...
	if ( $double_encode ) {
		...
		// Decode &amp; into &
		$string = asc_specialchars_decode( $string, $_quote_style );
 ...
 * @param mixed $quote_style Optional. Converts double quotes if set to ENT_COMPAT, both single and double if set to ENT_QUOTES or none if set to ENT_NOQUOTES. Also compatible with old _asc_specialchars() values; converting single quotes if set to 'single', double if set to 'double' or both if otherwise set. Default is ENT_NOQUOTES.
 * @return string The decoded text without HTML entities.
 */
function asc_specialchars_decode( $string, $quote_style = ENT_NOQUOTES ) {
	$string = (string) $string;
	...
	// Match the previous behaviour of _asc_specialchars() when the $quote_style is not an accepted value
	if ( empty( $quote_style ) ) {
		$quote_style = ENT_NOQUOTES;
	} elseif ( !in_array( $quote_style, array( 0, 2, 3, 'single', 'double' ), true ) ) {
		$quote_style = ENT_QUOTES;
 ...
 */
function asc_check_invalid_utf8( $string, $strip = false ) {
	$string = (string) $string;
...
function sanitize_user( $username, $strict = false ) {
	...
	$username = asc_strip_all_tags( $username );
...
function convert_chars($content, $deprecated = '') {
	...
	// Fix Word pasting
	$content = strtr($content, $asc_htmltranswinuni);
 ...
 */
function asc_rel_nofollow( $text ) {
	// This is a pre save filter, so text is already escaped.
	$text = stripslashes($text);
	$text = preg_replace_callback('|<a (.+?)>|i', 'asc_rel_nofollow_callback', $text);
	$text = asc_slash($text);
	return $text;
}
 ...
 */
function asc_rel_nofollow_callback( $matches ) {
	$text = $matches[1];
 ...
 *
 * @since 0.71
 * @uses $asc_smiliessearch
 *
 * @param string $text Content to convert smilies from text.
 * @return string Converted content with text smilies replaced with images.
 */
function convert_smilies( $text ) {
	global $asc_smiliessearch;
	$output = '';
	if ( get_option( 'use_smilies' ) && ! empty( $asc_smiliessearch ) ) {
		...
		for ( $i = 0; $i < $stop; $i++ ) {
			...
			if ( '' ==  $ignore_block_element && strlen( $content ) > 0 && '<' != $content[0] ) {
				$content = preg_replace_callback( $asc_smiliessearch, 'translate_smiley', $content );
			}
 ...
 */
function asc_iso_descrambler($string) {
	/* this may only work with iso-8859-1, I'm afraid */
	if (!preg_match('#\=\?(.+)\?Q\?(.+)\?\=#i', $string, $matches)) {
		return $string;
	} else {
		$subject = str_replace('_', ' ', $matches[2]);
		$subject = preg_replace_callback('#\=([0-9a-f]{2})#i', '_asc_iso_convert', $subject);
		return $subject;
	}
}
 ...
 */
function _asc_iso_convert( $match ) {
...
function asc_trim_excerpt($text = '') {
	...
	if ( '' == $text ) {
		...
		$text = asc_trim_words( $text, $excerpt_length, $excerpt_more );
	}
	/**
	 ...
	 */
	return apply_filters( 'asc_trim_excerpt', $text, $raw_excerpt );
}
 ...
 */
function asc_trim_words( $text, $num_words = 55, $more = null ) {
	if ( null === $more )
		$more = __( '&hellip;' );
	$original_text = $text;
	$text = asc_strip_all_tags( $text );
	 ...
	 */
	return apply_filters( 'asc_trim_words', $text, $num_words, $more, $original_text );
}
 ...
 */
function asc_richedit_pre($text) {
	if ( empty( $text ) ) {
		/**
		 * Filter text returned for the rich text editor.
		 *
		 * This filter is first evaluated, and the value returned, if an empty string
		 * is passed to asc_richedit_pre(). If an empty string is passed, it results
		 * in a break tag and line feed.
		 *
		 * If a non-empty string is passed, the filter is evaluated on the asc_richedit_pre()
 ...
 */
function asc_htmledit_pre($output) {
	if ( !empty($output) )
...
function esc_js( $text ) {
	$safe_text = asc_check_invalid_utf8( $text );
	$safe_text = _asc_specialchars( $safe_text, ENT_COMPAT );
...
function esc_html( $text ) {
	$safe_text = asc_check_invalid_utf8( $text );
	$safe_text = _asc_specialchars( $safe_text, ENT_QUOTES );
	/**
 ...
 */
function asc_make_link_relative( $link ) {
 ...
 */
function asc_parse_str( $string, &$array ) {
	 ...
	 */
	$array = apply_filters( 'asc_parse_str', $array );
}
 ...
 *
 * @uses asc_pre_kses_less_than_callback in the callback function.
 * @since 2.3.0
 *
 * @param string $text Text to be converted.
 * @return string Converted text.
 */
function asc_pre_kses_less_than( $text ) {
	return preg_replace_callback('%<[^>]*?((?=<)|>|$)%', 'asc_pre_kses_less_than_callback', $text);
}
 ...
 */
function asc_pre_kses_less_than_callback( $matches ) {
...
function asc_sprintf( $pattern ) {
	...
	while ( $len > $start ) {
		...
		if ( $pattern[$start] == '%' ) {
			...
			/**
			 * Filter a fragment from the pattern passed to asc_sprintf().
			 *
			 ...
			 */
			$_fragment = apply_filters( 'asc_sprintf', $fragment, $arg );
 ...
 */
function asc_sprintf_l($pattern, $args) {
	// Not a match
	...
	/**
	 * Filter the translated delimiters used by asc_sprintf_l().
	 ...
	 */
	$l = apply_filters( 'asc_sprintf_l', array(
 ...
 * @return string The excerpt.
 */
function asc_html_excerpt( $str, $count, $more = null ) {
	if ( null === $more )
		$more = '';
	$str = asc_strip_all_tags( $str, true );
...
function _links_add_base($m) {
	...
	return $m[1] . '=' . $m[2] .
		( preg_match( '#^(\w{1,20}):#', $m[3], $protocol ) && in_array( $protocol[1], asc_allowed_protocols() ) ?
			$m[3] :
 ...
 * will return 'something'. asc_strip_all_tags will return ''
 *
 * @since 2.9.0
 *
 ...
 */
function asc_strip_all_tags($string, $remove_breaks = false) {
...
function sanitize_text_field($str) {
	$filtered = asc_check_invalid_utf8( $str );

	if ( strpos($filtered, '<') !== false ) {
		$filtered = asc_pre_kses_less_than( $filtered );
		// This will strip extra whitespace for us.
		$filtered = asc_strip_all_tags( $filtered, true );
	} else {
 ...
 * @return string
 */
function asc_basename( $path, $suffix = '' ) {
...
function capital_P_dangit( $text ) {
	...
	if ( 'the_title' === $current_filter || 'asc_title' === $current_filter )
...
function asc_spaces_regexp() {
	...
	if ( empty( $spaces ) ) {
		 ...
		 */
		$spaces = apply_filters( 'asc_spaces_regexp', '[\r\n\t ]|\xC2\xA0|&nbsp;' );
	}
