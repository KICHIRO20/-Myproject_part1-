...
(function(jQuery) {
	...
	jQuery.fn.tree = function (opts) {
		return this.each(function() {
			var conf = jQuery.extend({},opts);
			if(tree_component.inst && tree_component.inst[jQuery(this).attr('id')]) tree_component.inst[jQuery(this).attr('id')].destroy();
			if(conf !== false) new tree_component().init(this, conf);
		});
	};
	jQuery.tree_create = function() {
		return new tree_component();
	};
	jQuery.tree_focused = function() {
		return tree_component.inst[tree_component.focused];
	};
	jQuery.tree_reference = function(id) {
		return tree_component.inst[id] || null;
	};

	// rollback
	jQuery.tree_rollback = function(data) {
		for(var i in data) {
			...
			if(data[i].selected) {
				tmp.selected = jQuery("#" + data[i].selected);
				tmp.selected_arr = [];
				tmp.container
		...
		if(typeof tree_component.inst == "undefined") {
			...
			tree_component.mousedown = function(event) {
				...
				if(tree_component.drag_drop.droppable.length && tmp.is("." + tree_component.drag_drop.droppable.join(", .")) ) {
					tree_component.drag_drop.drag_help	= jQuery("<div id='jstree-dragged' class='tree tree-default'><ul><li class='last dragged foreign " + event.target.className + "'><a href='#'>" + tmp.text() + "</a></li></ul></div>");
			...
			tree_component.mouseup = function(event) {
				...
				tmp.moving		= false;
				tmp.appended	= false;
				jQuery("li.dragged").removeClass("dragged");
			...
			tree_component.mousemove = function(event) {
				...
				if(tmp.isdown) {
					...
					if(tmp.open_time) clearTimeout(tmp.open_time);
					if(!tmp.appended) {
						if(tmp.foreign !== false) tmp.origin_tree = jQuery.tree_focused();
						jQuery("body").append(tmp.drag_help);
					...
					if(event.target.tagName == "A" ) {
						...
						if(ok) {
							...
							tmp.move_type	= mov;
							tmp.ref_node	= jQuery(event.target);
		...
		return {
			...
			settings : {
				data	: {
					...
					async_data : function (NODE, TREE_OBJ) { return { id : jQuery(NODE).attr("id") || 0 } }, // PARAMETERS PASSED TO SERVER
			...
			init : function(elem, conf) {
				...
				if(this.settings.path == false) {
					...
					jQuery("script").each( function () { 
						if(this.src.toString().match(/tree_component.*?jsjQuery/)) {
							_this.path = this.src.toString().replace(/tree_component.*?jsjQuery/, "");
						}
					});
				}
				...
				if(this.settings.rules.draggable != "none" && tree_component.drag_drop.marker === false) {
					var _this = this;
					tree_component.drag_drop.marker = jQuery("<img>")
						.attr({
							id		: "marker", 
			...
			refresh : function (obj) {
				...
				if(this.selected) {
					...
					if(obj) {
						jQuery(obj).find("li:has(a.clicked)").each(function () {
							jQuerythis = jQuery(this);
							if(jQuerythis.attr("id")) _this.settings.selected.push("#" + jQuerythis.attr("id"));
						});
					}
					else {
					...
					for(var i in this.settings.languages) {
						...
						if(force || inner_attrib.length || a.get(0).style.backgroundImage.toString().length) {
							...
							if(this.settings.ui.theme_name == "themeroller" && a.children("ins").size()) {
								...
								var cls = false;
								jQuery.each(tmp.split(" "), function (i, val) {
			...
			show_context : function (obj) {
				this.context.show();
				var tmp = jQuery(obj).children("a:visible").offset();
			...
			attachEvents : function () {
				...
				jQuery("#" + this.container.attr("id") + " li")
					.live("click", function(event) { // WHEN CLICK IS ON THE ARROW
						if(event.target.tagName != "LI") return true;
						_this.off_height();
						if(event.pageY - jQuery(event.target).offset().top > _this.li_height) return true;
						_this.toggle_branch.apply(_this, [event.target]);
						event.stopPropagation();
						return false;
					});
				jQuery("#" + this.container.attr("id") + " li a")
					...
					.live("contextmenu.jstree", function (event) {
						...
						if(_this.context) {
							if(_this.context.append == false) {
								jQuery("body").append(_this.context);
				...
				if(_this.settings.ui.theme_name == "themeroller") {
					jQuery("#" + this.container.attr("id") + " li a").live("mouseout", function (event) {
						...
						.live("mousedown.jstree", function (event) {
							...
							if(_this.settings.rules.multiple != false && _this.selected_arr.length > 1 && obj.children("a:eq(0)").hasClass("clicked")) {
								...
								if(counter > 0) {
									...
									tree_component.drag_drop.isdown		= true;
									tree_component.drag_drop.drag_help	= jQuery("<div id='jstree-dragged' class='tree " + (_this.container.hasClass("tree-default") ? " tree-default" : "" ) + (_this.settings.ui.theme_name && _this.settings.ui.theme_name != "default" ? " tree-" + _this.settings.ui.theme_name : "" ) + "' />").append("<ul class='" + _this.container.children("ul:eq(0)").get(0).className + "' />");
									var tmp = jQuery(tree_component.drag_drop.drag_node.get(0).cloneNode(true));
								...
								if(_this.check("draggable", obj)) {
									...
									tree_component.drag_drop.drag_help	= jQuery("<div id='jstree-dragged' class='tree " + (_this.container.hasClass("tree-default") ? " tree-default" : "" ) + (_this.settings.ui.theme_name && _this.settings.ui.theme_name != "default" ? " tree-" + _this.settings.ui.theme_name : "" ) + "' />").append("<ul class='" + _this.container.children("ul:eq(0)").get(0).className + "' />");
									var tmp = jQuery(obj.get(0).cloneNode(true));
									if(_this.settings.languages.length > 0) tmp.find("a").not("." + _this.current_lang).hide();
									tree_component.drag_drop.drag_help.children("ul:eq(0)").append(tmp);
				...
				if(_this.context) jQuery(document).bind("mousedown", function() { _this.hide_context(); });
			},
			checkMove : function (NODES, REF_NODE, TYPE) {
				...
				if(this.settings.rules.use_inline && this.settings.rules.metadata) {
					...
					if(nd.size()) {
						...
						if((typeof jQuery(nd).metadata().max_depth).toLowerCase() != "undefined" && jQuery(nd).metadata().max_depth < incr) ok = false;
						else {
							nd.parents("li").each(function(i) {
								if(ok == false) return false;
								if((typeof jQuery(this).metadata().max_depth).toLowerCase() != "undefined") {
			...
			reselect : function (is_callback) {
				...
				if(this.settings.selected !== false) {
					jQuery.each(this.settings.selected, function (i) {
						if(_this.is_partial_refresh)	_this.select_branch(jQuery(_this.settings.selected[i], _this.container), (_this.settings.rules.multiple !== false) );
						else							_this.select_branch(jQuery(_this.settings.selected[i], _this.container), (_this.settings.rules.multiple !== false && i > 0) );
					});
			...
			create : function (obj, ref_node, position) { 
				...
				var torename = false; 
				if(!obj)	obj = {};
				else		obj = jQuery.extend(true, {}, obj);
				...
				var jQueryli = jQuery(this.parseJSON(obj));
				if(jQueryli.children("ul").size()) {
					if(!jQueryli.is(".open")) jQueryli.addClass("closed");
				}
				else jQueryli.addClass("leaf");
				jQueryli.find("li:last-child").addClass("last").end().find("li:has(ul)").not(".open").addClass("closed");
				jQueryli.find("li").not(".open").not(".closed").addClass("leaf");

				if(!root && this.settings.rules.use_inline && this.settings.rules.metadata) {
					var t = this.get_type(jQueryli) || "";
					jQuery.metadata.setType("attr", this.settings.rules.metadata);
					if(typeof ref_node.metadata()["valid_children"] != "undefined") {
						if(jQuery.inArray(t, ref_node.metadata()["valid_children"]) == -1) return this.error("CREATE: NODE NOT A VALID CHILD");
					}
					...
					var ok = true;
					if((typeof jQuery(ref_node).metadata().max_depth).toLowerCase() != "undefined" && jQuery(ref_node).metadata().max_depth === 0) ok = false;
					else {
				...
				if(ref_node.children("ul").size() == 0 || (root == true && ref_node.children("ul").children("li").size() == 0) ) {
					if(!root)	var a = this.moved(jQueryli,ref_node.children("a:eq(0)"),"inside", true);
					else		var a = this.moved(jQueryli,this.container.children("ul:eq(0)"),"inside", true);
				}
				else if(pos == "before" && ref_node.children("ul:eq(0)").children("li:nth-child(" + (position + 1) + ")").size())
					var a = this.moved(jQueryli,ref_node.children("ul:eq(0)").children("li:nth-child(" + (position + 1) + ")").children("a:eq(0)"),"before", true);
				else if(pos == "after" &&  ref_node.children("ul:eq(0)").children("li:nth-child(" + (position) + ")").size())
					var a = this.moved(jQueryli,ref_node.children("ul:eq(0)").children("li:nth-child(" + (position) + ")").children("a:eq(0)"),"after", true);
				else if(ref_node.children("ul:eq(0)").children("li:nth-child(" + (position + 1) + ")").size())
					var a = this.moved(jQueryli,ref_node.children("ul:eq(0)").children("li:nth-child(" + (position + 1) + ")").children("a:eq(0)"),"before", true);
				else
					var a = this.moved(jQueryli,ref_node.children("ul:eq(0)").children("li:last").children("a:eq(0)"),"after",true);

				if(a === false) return this.error("CREATE: ABORTED");

				if(torename) {
					this.select_branch(jQueryli.children("a:eq(0)"));
					this.rename();
				}
				return jQueryli;
			},
			rename : function (obj) {
				...
				_this.inp = jQuery("<input type='text' autocomplete='off' />");
				_this.inp
				...
				_this.inp.blur(function(event) {
						...
						jQuery(obj).text( jQuery(obj).parent().find("input").eq(0).attr("value") ).get(0).style.display = ""; 
			...
			remove : function(obj) {
				...
				if(obj && (!this.selected || this.get_node(obj).get(0) != this.selected.get(0) )) {
					...
					if(obj.size()) {
						...
						jQueryparent = obj.parent();
						obj = obj.remove();
						jQueryparent.children("li:last").addClass("last");
						if(jQueryparent.children("li").size() == 0) {
							jQueryli = jQueryparent.parents("li:eq(0)");
							jQueryli.removeClass("open").removeClass("closed").addClass("leaf").children("ul").remove();
							this.set_cookie("open");
						}
					...
					jQueryparent = this.selected.parent();
			...
			set_cookie : function (type) {
				...
				switch(type) {
					...
					case "open":
						...
						jQuery.cookie(this.settings.cookies.prefix + '_open',str.replace(/,jQuery/ig,""),this.settings.cookies.opts);
						break;
				}
			},
			...
			moved : function (what, where, how, is_new, is_copy, rb) {
				...
				var jQueryparent	= jQuery(what).parents("ul:eq(0)");
				var jQuerywhere	= jQuery(where);
				...
				if(how == "inside" && this.settings.data.async && this.get_node(jQuerywhere).hasClass("closed")) {
					var _this = this;
					return this.open_branch(this.get_node(jQuerywhere), true, function () { _this.moved.apply(_this, [what, where, how, is_new, is_copy, rb]); });
				}
				...
				if(is_copy) {
					...
					_what.each(function (i) {
						...
						jQuery(this).removeClass("dragged").find("a.clicked").removeClass("clicked ui-state-active").end().find("li.dragged").removeClass("dragged");
					});
				}
				else _what = what;
				if(is_new) {
				...
				switch(how) {
					case "before":
						jQuerywhere.parents("ul:eq(0)").children("li.last").removeClass("last");
						jQuerywhere.parent().before(what.removeClass("last"));
						jQuerywhere.parents("ul:eq(0)").children("li:last").addClass("last");
						break;
					case "after":
						jQuerywhere.parents("ul:eq(0)").children("li.last").removeClass("last");
						jQuerywhere.parent().after(what.removeClass("last"));
						jQuerywhere.parents("ul:eq(0)").children("li:last").addClass("last");
						break;
					case "inside":
						if(jQuerywhere.parent().children("ul:first").size()) {
							if(this.settings.rules.createat == "top")	jQuerywhere.parent().children("ul:first").prepend(what.removeClass("last")).children("li:last").addClass("last");
							else										jQuerywhere.parent().children("ul:first").children(".last").removeClass("last").end().append(what.removeClass("last")).children("li:last").addClass("last");
						}
						else {
							what.addClass("last");
							jQuerywhere.parent().append("<ul/>").removeClass("leaf").addClass("closed");
							jQuerywhere.parent().children("ul:first").prepend(what);
						}
						if(jQuerywhere.parent().hasClass("closed")) { this.open_branch(jQuerywhere); }
						break;
					default:
						break;
				}
				// CLEANUP OLD PARENT
				if(jQueryparent.find("li").size() == 0) {
					var jQueryli = jQueryparent.parent();
					jQueryli.removeClass("open").removeClass("closed").addClass("leaf");
					if(!jQueryli.is(".tree")) jQueryli.children("ul").remove();
					jQueryli.parents("ul:eq(0)").children("li.last").removeClass("last").end().children("li:last").addClass("last");
					this.set_cookie("open");
				}
				else {
					jQueryparent.children("li.last").removeClass("last");
					jQueryparent.children("li:last").addClass("last");
				}
				...
				else if(is_new)	this.settings.callback.oncreate.call(null, this.get_node(what).get(0), (jQuerywhere.is("ul") ? -1 : this.get_node(where).get(0) ), how, this, rb);
			...
			search : function(str) {
				...
				if(this.settings.data.async) {
					if(!this.srch_opn) {
						var dd = jQuery.extend( { "search" : str } , this.settings.data.async_data(false, this) );
						jQuery.ajax({
							type		: this.settings.data.method,
							url			: this.settings.data.url, 
							data		: dd, 
			...
			destroy : function() {
				...
				jQuery("#" + this.container.attr("id")).die("click.jstree").die("dblclick.jstree").die("contextmenu.jstree").die("mouseover.jstree").die("mouseout.jstree").die("mousedown.jstree");
