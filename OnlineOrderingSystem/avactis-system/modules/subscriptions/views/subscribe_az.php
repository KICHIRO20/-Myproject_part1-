...
//------------------------------------------------
    ...
    function Subscriptions_Subscribe()
    {
        global $application;
    ...
    function outputAskEmails()
    {
        ...
        $vars = array(
                ...
                'DBEmailsDisabled' => $this->_emails_from == 'db' ? '' : 'disabled',
                ...
                'DBEmailsChecked' => $this->_emails_from == 'db' ? 'checked' : '',
                'TypedEmails' => @$this->POST['typed_emails'],

                'Action' => $this->_prefix,
                'WinName' => $this->_Prefix,
                'WinTitle' => $this->_messageResources->getMessage('TITLE_'.$this->_PREFIX),
                'TipTopics' => $this->_messageResources->getMessage('TIP_TOPICS_'.$this->_PREFIX),
                'TipEmails' => $this->_messageResources->getMessage('TIP_EMAILS_'.$this->_PREFIX),
                );
        $result = $this->_tmplFiller->fill('subscribe/', 'box.tpl.html', $vars);
		$topic_list = $this->getTopicsList();
		if(empty($topic_list))
			$result = $this->_tmplFiller->fill('subscribe/', 'box.empty.tpl.html');
        return $result;
    }
    ...
    function outputAskConfirmation()
    {
        ...
        $vars = array(
                ...
                //'WinName' => $this->_Prefix,
                //'WinTitle' => $this->_messageResources->getMessage('TITLE_'.$this->_PREFIX),
                'TipRecheck' => $this->_messageResources->getMessage('TIP_TOPICS_RECHECK_'.$this->_PREFIX),
                );
        $result = $this->_tmplFiller->fill($this->_prefix.'/', 'box.confirm.tpl.html', $vars);
        return $result;
    ...
    function getConfirmTopicsList()
    {
        ...
        foreach ($topics as $topic) {
            ...
            $res .= $this->_tmplFiller->fill($this->_prefix.'/', 'item.confirm.tpl.html', $vars);
        }
