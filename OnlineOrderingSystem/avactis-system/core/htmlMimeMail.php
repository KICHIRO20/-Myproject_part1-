...
*/
	function &_addTextPart(&$obj, $text)
	{
		...
		if (is_object($obj)) {
           ...
           $obj = new Mail_mimePart($text, $params);
			return $obj;
		}
	}
...
*/
	function buildMessage($params = array())
	{
		...
		if (isset($message)) {
			...
			$message_id = sprintf('<%s.%s@%s>', base_convert(time(), 10, 36), base_convert(rand(), 10, 36), !empty($GLOBALS['_SERVER']['HTTP_HOST']) ? $GLOBALS['_SERVER']['HTTP_HOST'] : $GLOBALS['_SERVER']['SERVER_NAME']);
...
*/
	function _encodeHeader($input, $charset = 'ISO-8859-1')
	{
		...
		foreach ($matches[1] as $value) {
			$replacement = preg_replace('/([\x80-\xFF])/e', '"=" . _ml_strtoupper(dechex(_byte_ord("\1")))', $value);
...
//remove//		$smtp = &smtp::connect($this->smtp_params);
                ...
                $mail->Host = $this->smtp_params['host'];				// Specify main and backup server
                $mail->Port = $this->smtp_params['port'];				// SMTP port
//                $mail->SMTPSecure = 'ssl';								// Enable encryption, 'ssl' also accepted
                $mail->SMTPAuth = $this->smtp_params['auth'];			// Enable SMTP authentication
                $mail->Username = $this->smtp_params['user'];			// SMTP username
                $mail->Password = $this->smtp_params['pass'];			// SMTP password
				$mail->Subject = $this->headers['Subject'];
				$isHTML = !empty($this->html)? true : false;
		        $mail->IsHTML($isHTML);									// sets ContentType to text or html
